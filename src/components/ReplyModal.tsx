...
const ReplyModal: React.FC<ReplyModalProps> = ({
  ...
  const renderOrderedContent = (content: any) => {
    ...
    // Try to parse as ordered content array
    let orderedContent;
    try {
  ...
  // Get original content (post or comment)
  const originalContent = originalComment || originalPost
  const originalUser = originalContent?.user
              ...
              {originalContent && (
                <Box sx={{
                  ...
                  <Box sx={{ display: 'flex', gap: 2 }}>
                    <Avatar
                      src={originalUser?.profileImage || undefined}
                      sx={{
                        width: 48,
                        height: 48,
                    ...
                    >
                      {originalUser?.nickname?.charAt(0)}
                    </Avatar>
                    ...
                    <Box sx={{ flex: 1 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                        ...
                        <Typography variant="caption" color="text.secondary" sx={{ fontSize: '14px', color: '#666' }}>
                          {getTimeAgo(originalContent.createdAt)}
                        </Typography>
                      </Box>

                      {renderOrderedContent(originalContent.content)}
